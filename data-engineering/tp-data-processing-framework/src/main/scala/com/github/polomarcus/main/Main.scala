...
object Main {
  def main(args: Array[String]) {
    ...
    // Tips: use a groupBy
    filteredNewsAboutClimate.groupBy($"containsWorldGlobalWarming").count().show()
    ...
    val climateRelatedNewsCount = spark.sql("SELECT COUNT(*) FROM news WHERE containsWordGlobalWarming = true").show()
    val nonClimateRelatedNewsCount = spark.sql("SELECT COUNT(*) FROM news WHERE containsWordGlobalWarming = false").show()
